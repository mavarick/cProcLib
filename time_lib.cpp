...
/*
 * struct tm ->
 * time_t, long, micro-seconds from 1970 :: mktime(const struct tm &tm_)
...
time_t str2ts(string str, const string format){
    struct tm tm_;
    strptime(str.data(), format.data(), &tm_);
    return mktime(&tm_);
}
...
//"%Y-%m-%d %H:%M:%S"
// timestamp -> string
string ts2str(time_t ts, const string format){
    char buf [256];
    tm * tm_ = localtime(&ts);
